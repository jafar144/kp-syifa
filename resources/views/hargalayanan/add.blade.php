...
<head>
    ...
    <title>Halaman Tambah Harga Layanan</title>
</head>
<body>
    <hr>
    ...
    <form action="{{ url('hargaLayanan/add') }}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="harga" id="harga" placeholder="Masukkan harga Layanan" class="form-control my-2" value="{{ old('harga') }}">
            @error('harga')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
